...
for _ in range(m):
    cmd = input().split()
    if cmd[0] == 'L':
        # 왼쪽으로 이동: 현재 가리키는 노드가 head가 아니어야 함
        if it != l.begin():
            it = it.prev

    elif cmd[0] == 'R':
        # 오른쪽으로 이동: 현재 가리키는 노드가 tail(dummy)이 아니어야 함
        if it != l.end():
            it = it.next

    elif cmd[0] == 'D':
        # 삭제: 현재 it가 가리키는 노드를 삭제하고 it를 다음 노드로 이동
        # 단, dummy 노드(l.end())는 삭제하면 안 됨
        if it != l.end():
            it = l.erase(it)
        
    elif cmd[0] == 'P': # 문자를 추가하는 명령어라고 가정 (예: P x)
        # 현재 it가 가리키는 위치 바로 앞에 삽입
        l.insert(it, cmd[1])
        # 삽입 후 it는 그대로 두거나 필요에 따라 이동
